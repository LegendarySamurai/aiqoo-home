...
function setUtmParam(name) {
    if (getUrlParams(name, window.location.href) !== null)
    {
        ...
        if (res.length > 1){setCookie(name, res, 168);}
    }
}
...
function populateUserObject() {
    ...
    _userObj.brwoser_name = checkBrowser(),
        _userObj.browser_version = checkVersionBrowser(),
        _userObj.platform = navigator.platform,
        _userObj.operation_system = navigator.appVersion,
        _userObj.connection = GetConnection(),
        _userObj.connection_speed = ConnectionSpeed(),
        _userObj.url = window.location.href,
        _userObj.page_refer = document.referrer,
        _userObj.device_operation_system = getDeviceOperatingSystem(),
        _userObj.device = getDevice(),
        _userObj.device_height = screen.height,
        _userObj.device_width = screen.width,
        _userObj.document_height = window.innerHeight,
        _userObj.document_width = window.innerWidth,
        _userObj.page_open_time = new Date(),
        _userObj.utm_source = getUrlParams('utm_source');
